...
void CPlayer::Move(const DWORD dwDirection, float fDistance)
{
	if (dwDirection)
	{
		...
		if (dwDirection & DIR_FORWARD) 
			XMStoreFloat3(&xmf3Shift, XMVectorAdd(XMLoadFloat3(&xmf3Shift), XMVectorScale(XMLoadFloat3(&m_xmf3Look), fDistance)));
		if (dwDirection & DIR_BACKWARD)
			XMStoreFloat3(&xmf3Shift, XMVectorAdd(XMLoadFloat3(&xmf3Shift),	XMVectorScale(XMLoadFloat3(&m_xmf3Look), -fDistance)));
		if (dwDirection & DIR_RIGHT)
			XMStoreFloat3(&xmf3Shift, XMVectorAdd(XMLoadFloat3(&xmf3Shift),	XMVectorScale(XMLoadFloat3(&m_xmf3Right), fDistance)));
...
void CPlayer::Move(const XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
	...
	if (m_xmf3Position.y > 30.0f || m_xmf3Position.y < -30.0f) {
		m_xmf3Position = XMFLOAT3(0.0f, 0.0f, 0.0f);
	}
	if (m_xmf3Position.z > 20.0f || m_xmf3Position.z < -30.0f) {
		m_xmf3Position = XMFLOAT3(0.0f, 0.0f, 0.0f);
	}
...
// 플레이어의 위치를 받아 그 지점부터 발사 시작
//XMFLOAT3 CPlayer::GetPosition(CPlayer& pPlayer)
//{
//	return pPlayer.m_xmf3Position;
//}