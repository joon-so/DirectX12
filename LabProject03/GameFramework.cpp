...
void CGameFramework::BuildObjects()
{
	...
	m_pPlayer->SetPosition(10.0f, 0.0f, 0.0f);
	...
	m_pScene->BuildObjects(*m_pPlayer);
}
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_CONTROL:		
			//ÃÑ¾Ë ¹ß»ç
			if (m_pScene->m_nObjects + m_pScene->m_nBullets < m_pScene->mObjectCount) {
				m_pScene->BuildBullet(*m_pPlayer, m_pScene->m_nBullets);
				m_pScene->m_nBullets++;
			}
			else {
				m_pScene->m_nBullets = 0;
			}
			break;
		default:
